...
- **tmux**: a persistent session auto-attached on SSH login — with directory-labelled tabs and a live AI status bar (auto-detects Claude, Codex, Agy; the bar fills as the 5h session limit is used, followed by context tokens and cost, sourced from Claude Code's statusLine hook; activity `⚡` for Codex and Agy)
