...
run "tailscale_no_cidr_closes_ssh" {
  ...
  assert {
    ...
    error_message = "local.ssh_cidrs must be empty in Azure when tailscale_authkey is set without ssh_allowed_cidrs"
  }
}
...
run "tailscale_oauth_only_closes_ssh" {
  ...
  assert {
    condition     = length([for r in azurerm_network_security_group.this.security_rule : r if r.name == "Tailscale-WireGuard"]) > 0
    error_message = "Tailscale WireGuard security rule must be present when tailscale_enabled is true"
  }
}
