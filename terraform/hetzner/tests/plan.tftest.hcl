...
run "tailscale_oauth_only_closes_ssh" {
  ...
  assert {
    condition     = length([for r in hcloud_firewall.this.rule : r if r.description == "Tailscale WireGuard"]) > 0
    error_message = "Tailscale WireGuard rule must be present when tailscale_enabled is true"
  }
}
