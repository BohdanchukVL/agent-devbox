...
if [ -n "$sid" ] && mkdir -p "$STATE_DIR" 2>/dev/null; then
  ...
  if printf '%s\n' "$input" | jq -c . > "$tmp" 2>/dev/null; then
    ...
    cp -f "$STATE_DIR/$sid.json" "$STATE_DIR/latest.json" 2>/dev/null || true
  else
    rm -f "$tmp"
  fi
...
# Render status row: prefer ccusage statusline if available, fallback to native jq summary
rendered=""
if command -v ccusage >/dev/null 2>&1; then
  rendered=$(printf '%s\n' "$input" | ccusage statusline --visual-burn-rate emoji 2>/dev/null || true)
fi

if [ -n "$rendered" ]; then
  printf '%s\n' "$rendered"
else
  # Status row fallback: model · ctx N% · 5h N% · 7d N% · $cost
